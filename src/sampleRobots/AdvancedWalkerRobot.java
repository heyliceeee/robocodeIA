...
public class AdvancedWalkerRobot extends AdvancedRobot {
  ...
  public void onMouseClicked(MouseEvent e) {
    ...
    for (int i = 0; i < size; i++) points.add(
      new Point(rand.nextInt(conf.getWidth()), rand.nextInt(conf.getHeight()))
    );
  ...
  public void onPaint(Graphics2D g) {
    ...
    obstacles
      .stream()
      .forEach(x ->
        g.drawRect(x.x, x.y, (int) x.getWidth(), (int) x.getHeight())
      );

    if (points != null) {
      for (int i = 1; i < points.size(); i++) drawThickLine(
        g,
        points.get(i - 1).getX(),
        points.get(i - 1).getY(),
        points.get(i).getX(),
  ...
  public void onScannedRobot(ScannedRobotEvent event) {
    ...
    Point2D.Double ponto = getEnemyCoordinates(
      this,
      event.getBearing(),
      event.getDistance()
    );
    ...
    if (
      inimigos.containsKey(event.getName())
    ) obstacles.remove(inimigos.get(event.getName())); //se já existe um retângulo deste inimigo //remover da lista de retângulos
  ...
  public static void advancedRobotGoTo(
    ...
    if (targetAngle == turnAngle) robot.setAhead(distance); else robot.setBack(
      distance
    );
    robot.execute();
  }
